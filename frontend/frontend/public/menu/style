...
/** Netlify Image CDN URL — resizes and format-negotiates at the edge. */
const img = (file, w, q = 75) =>
  `/.netlify/images?url=${encodeURIComponent(MENU_DIR + file)}&w=${w}&q=${q}`;
    ...
    (m, i) => `
      <figure class="shot reveal" data-i="${i}" style="transition-delay:${Math.min(i, 9) * 60}ms">
        <button type="button" class="shot-btn" aria-label="View ${m.title} menu board full size">
          <img src="${img(m.file, 600)}" alt="${m.title} drink menu board" loading="lazy" decoding="async" width="600" height="750" />
...
function showSlide(i) {
  ...
  el.lbImg.src = img(m.file, 1400, 82);
